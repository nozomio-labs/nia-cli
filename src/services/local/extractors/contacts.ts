...
function extractFromSingleContactsDb(
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const displayName = nameParts.length > 0 ? nameParts.join(" ") : (row.organization ?? "Unknown");
...
export function extractContacts(
  ...
  for (const singleDbPath of dbPaths) {
    const { maxModifiedAt } = extractFromSingleContactsDb(singleDbPath, cursor, files);
  ...
  return {
    ...
    stats: { db_type: "contacts", extracted: files.length, db_count: dbPaths.length },
  };
}
