...
type VaultMode = "ingest" | "sync" | "lint" | "refresh" | "dream";
...
const dreamCommand = makeRunCommand(
	"dream",
	"Self-improving dream cycle: discover entities mentioned across pages that lack their own page, synthesize new pages, find non-obvious connections between pages from different sources, detect contradictions, and write dream-report.md",
);
...
const autoDreamCommand = app
	.sub("auto-dream")
	.meta({
		description:
			"Toggle the weekly dream cycle cron for a vault. Runs at 03:00 UTC every Sunday when enabled.",
	})
	.args([
		{ name: "id", type: "string", description: "Vault ID", required: true },
		{
...
nia vault dream ${vaultId}    # self-improving cycle: discover entities, find connections, enrich
...
**Wiki layout** (every vault has these): \`/schema.md\` (user-owned conventions, never overwritten), \`/index.md\` (auto-maintained catalog), \`/log.md\` (append-only history), \`/META.md\` (snapshot), \`/concepts/*.md\` and \`/entities/*.md\` (LLM-generated, regenerated by sync), \`/notes/*.md\` (user-curated, sync always respects), \`/lint-report.md\` (latest lint output), \`/dream-report.md\` (latest dream cycle output — entities discovered, connections found, contradictions).

**Page structure**: Pages use a dual-section format separated by \`---\`: **Compiled Truth** (above, rewritten on sync) and **Timeline** (below, append-only evidence trail). Cross-links use \`[[Title]]\` or typed wikilinks \`[[Title|type]]\` (types: relates_to, works_at, founded, uses, extends, contradicts, supports, part_of).

**Dream cycle** (\`nia vault dream\`): Self-improving loop that scans existing pages, discovers entities mentioned but lacking their own page, synthesizes new pages, finds non-obvious connections between pages from different sources, and writes \`/dream-report.md\`. Enable weekly auto-dream via \`nia vault auto-dream ${vaultId} on\`.
...
- \`/dream-report.md\` — latest output of \`nia vault dream\` (entities discovered, connections found, contradictions).

## Page structure

Pages use a dual-section format separated by \`---\`:
- **Compiled Truth** (above ---): Current best understanding, present tense. Rewritten by sync when sources change.
- **Timeline** (below ---): Append-only evidence trail. Each entry: \`- **YYYY-MM-DD** | [summary] [Source: name]\`.

Cross-links use \`[[Title]]\` or typed wikilinks \`[[Title|type]]\` for entity relationships. Supported types: \`relates_to\` (default), \`works_at\`, \`founded\`, \`uses\`, \`extends\`, \`contradicts\`, \`supports\`, \`part_of\`.

## Dream cycle

\`nia vault dream ${vaultId}\` runs a self-improving loop: discovers entities mentioned across pages that lack their own page, synthesizes new pages from context, finds non-obvious connections between pages from different sources, detects contradictions, and writes \`/dream-report.md\`. Enable weekly automatic runs via \`nia vault auto-dream ${vaultId} on\`.
...
export const vaultCommand = annotate(
	...
	[
		...
		"`nia vault dream <id>` runs the self-improving dream cycle: discovers entities, finds connections across sources, detects contradictions, writes dream-report.md. Enable weekly auto-dream via `nia vault auto-dream <id> on`.",
	],
);
