...
const DOCTOR_PATHS: Record<string, string> = {
	...
	chrome_history: path.join(homedir(), "Library/Application Support/Google/Chrome/Default/History"),
	whatsapp: path.join(homedir(), "Library/Group Containers/group.net.whatsapp.WhatsApp.shared/ChatStorage.sqlite"),
	notes: path.join(homedir(), "Library/Group Containers/group.com.apple.notes/NoteStore.sqlite"),
	screen_time: path.join(homedir(), "Library/Application Support/Knowledge/knowledgeC.db"),
	photos: path.join(homedir(), "Pictures/Photos Library.photoslibrary/database/Photos.sqlite"),
	podcasts: path.join(homedir(), "Library/Group Containers/243LU875E5.groups.com.apple.podcasts/Documents/MTLibrary.sqlite"),
};
...
const doctorCommand = app
	...
	.run(async ({ flags }) => {
		...
		await withErrorHandling({ domain: "Doctor" }, async () => {
			...
			try {
				const sources = await listLocalSources(flags["api-key"]);
				checks.push({ source: "API", status: "OK", detail: `${sources.length} source(s) found` });
			} catch (err) {
				checks.push({ source: "API", status: "FAIL", detail: err instanceof Error ? err.message : "Connection failed" });
			}

			for (const [name, dbPath] of Object.entries(DOCTOR_PATHS)) {
				if (!existsSync(dbPath)) {
					checks.push({ source: name, status: "SKIP", detail: "Not installed" });
					continue;
				}
				try {
					...
					checks.push({ source: name, status: "FAIL", detail: "Permission denied — grant Full Disk Access" });
				}
			}
